...
function renderCouponRow(coupon){
  var c = coupon || {};
  return '' +
'<div class="promo-coupon-row" data-id="' + esc(c.id || '') + '" style="display:grid;grid-template-columns:auto 1.2fr 1.5fr 1fr 1fr 1fr 1fr auto auto;gap:6px;align-items:center;padding:8px;background:#f8fafc;border-radius:6px">' +
...
'  <select class="cp-reward" title="回饋方式：轉點數(下次用) 或 直接折扣(當次折現金)" style="padding:6px;border:1px solid #cbd5e1;border-radius:4px">' +
'    <option value="points"' + (c.rewardMode === 'cash' ? '' : ' selected') + '>轉點數</option>' +
'    <option value="cash"' + (c.rewardMode === 'cash' ? ' selected' : '') + '>直接折扣</option>' +
'  </select>' +
...
function collectFormToConfig(){
  ...
  document.querySelectorAll('#promoCouponList .promo-coupon-row').forEach(function(row){
    ...
    coupons.push({
      ...
      rewardMode: (row.querySelector('.cp-reward') && row.querySelector('.cp-reward').value === 'cash') ? 'cash' : 'points'
    });
